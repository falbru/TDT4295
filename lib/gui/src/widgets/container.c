...
static void update_hbox_layout(Widget *container)
{
    ...
    int available_width = container->width - (2 * data->padding) - ((data->child_count - 1) * data->spacing);
    ...
    for (int i = 0; i < data->child_count; i++)
    {
        ...
        int child_height = available_height;

        int child_y = container->y + data->padding;

        if (data->alignment == ALIGN_CENTER)
        {
            child_y = container->y + (container->height - child_height) / 2;
        }
        else if (data->alignment == ALIGN_END)
        {
            child_y = container->y + container->height - data->padding - child_height;
        }
        else if (data->alignment != ALIGN_STRETCH && child->height > 0)
        {
...
static void update_vbox_layout(Widget *container)
{
    ...
    int available_height = container->height - (2 * data->padding) - ((data->child_count - 1) * data->spacing);
    ...
    for (int i = 0; i < data->child_count; i++)
    {
        ...
        int child_width = available_width;
        int child_height = child->height > 0 ? child->height : flexible_height;

        int child_x = container->x + data->padding;

        if (data->alignment == ALIGN_CENTER)
        {
            child_x = container->x + (container->width - child_width) / 2;
        }
        else if (data->alignment == ALIGN_END)
        {
            child_x = container->x + container->width - data->padding - child_width;
        }
        else if (data->alignment != ALIGN_STRETCH && child->width > 0)
        {
