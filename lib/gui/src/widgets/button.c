...
static void button_render_callback(Widget *widget, Framebuffer *framebuffer) {
    ...
    renderFilledRectangle(widget->x, widget->y, widget->width, widget->height,
                          data->background_color, framebuffer);

    if (data->border_thickness > 0) {
        renderRectangle(widget->x, widget->y, widget->width, widget->height,
...
void button_set_text(Widget *button, const char *text) {
    if (!button || button->type != WIDGET_TYPE_BUTTON) return;
...
const char *button_get_text(Widget *button) {
    if (!button || button->type != WIDGET_TYPE_BUTTON) return NULL;
