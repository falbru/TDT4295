...
#define BUTTON_PLAY_BG COLOR_RGB(46, 170, 80)
#define BUTTON_PLAY_BORDER COLOR_RGB(35, 118, 54)
...
Widget *menu_page_init(const GameConfig *config)
{
    ...
    container_set_animation_speed(g_menu.label_title, 60);

    const Image *title_chars[] = {&s_image, &k_image, &i_image, &d_image, &a_image,
                                  &d_image, &d_image, &l_image, &e_image};
    for (int i = 0; i < 9; i++)
    {
    ...
    button_set_background_color(g_menu.button_play, BUTTON_PLAY_BG);
    button_set_text_color(g_menu.button_play, COLOR_WHITE);
    button_set_border(g_menu.button_play, BUTTON_PLAY_BORDER, 1);
    button_set_on_click(g_menu.button_play, game_on_play, NULL);
