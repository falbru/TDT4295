...
#define GTZC_MPCBB_NB_LCK_VCTR_REG_MAX (1U) /* One 32-bit needed           */
    ...
    typedef struct
    {
        uint32_t MPCBB_SecConfig_array[GTZC_MPCBB_NB_VCTR_REG_MAX];  /*!< Each element specifies secure access mode for
                                                                          ...
                                                                          1 means secure */
        uint32_t MPCBB_PrivConfig_array[GTZC_MPCBB_NB_VCTR_REG_MAX]; /*!< Each element specifies privilege access mode
                                                                        for a super-block. Each bit corresponds to a
                                                                        block inside the super-block. 0 means
                                                                        non-privilege, 1 means privilege */
        uint32_t MPCBB_LockConfig_array[GTZC_MPCBB_NB_LCK_VCTR_REG_MAX]; /*!< Each bit specifies the lock configuration
                                                                            of a super-block (32 blocks). 0 means
                                                                            unlocked, 1 means locked */
 ...
 */
    ...
    HAL_StatusTypeDef HAL_GTZC_MPCBB_GetLockConfig(uint32_t MemAddress, uint32_t NbSuperBlocks,
